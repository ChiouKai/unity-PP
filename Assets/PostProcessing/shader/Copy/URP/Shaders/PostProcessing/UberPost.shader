Shader "Hidden/Universal Render Pipeline/UberPost"
{
    Properties
    {
        _InternalLut("InternalLut", 2D) = "white" {}
    }
    HLSLINCLUDE
        ...
        #define ChromaCenter            _Chroma_Params.yz
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            #if _CHROMATIC_ABERRATION
            {
                ...
                float2 coords = 2.0 * uv - 1.0 - ChromaCenter;
                float2 end = - coords * dot(coords, coords) * ChromaAmount;//
                float2 delta = end / 3.0;
