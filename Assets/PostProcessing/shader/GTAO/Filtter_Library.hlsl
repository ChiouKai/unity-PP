...
//////Sharpe
inline half4 Sharpe(sampler2D sharpColor, half sharpness, half2 Resolution, half2 UV)
{
    half2 step = 1 / Resolution.xy;

    half3 texA = tex2D(sharpColor, UV + half2(-step.x, -step.y) * 1.5).rgb;
    half3 texB = tex2D(sharpColor, UV + half2(step.x, -step.y) * 1.5).rgb;
    half3 texC = tex2D(sharpColor, UV + half2(-step.x, step.y) * 1.5).rgb;
    half3 texD = tex2D(sharpColor, UV + half2(step.x, step.y) * 1.5).rgb;
...
inline half4 BilateralClearUp(sampler2D Color, half2 Resolution, half2 uv)
{
    ...
    for (int i = -kernelSize; i <= kernelSize; i++)
    {
        ...
        for (int j = -kernelSize; j <= kernelSize; j++)
        {
            ...
            weight = CrossBilateralWeight_2(blurColor.xyz - originColor.xyz, Blur_Radius) * kernel[kernelSize + j] * kernel[kernelSize + i];
            Num_Weight += weight;
...
inline void ResolverAABB(TEXTURE2D(currColor), SAMPLER(samplerName), half Sharpness, half ExposureScale, half AABBScale, half2 uv, half2 screenSize, inout half4 minColor, inout half4 maxColor, inout half4 filterColor)
{
    half4 TopLeft = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(-1, -1) / screenSize));
    half4 TopCenter = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(0, -1) / screenSize));
    half4 TopRight = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(1, -1) / screenSize));
    half4 MiddleLeft = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(-1,  0) / screenSize));
    half4 MiddleCenter = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(0,  0) / screenSize));
    half4 MiddleRight = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(1,  0) / screenSize));
    half4 BottomLeft = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(-1,  1) / screenSize));
    half4 BottomCenter = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(0,  1) / screenSize));
    half4 BottomRight = SAMPLE_TEXTURE2D(currColor, samplerName, uv + (int2(1,  1) / screenSize));
